...
study of short-horizon predictability built on top of it. Rust does the data
path, Python does the statistics.

A full Nasdaq session — 282 million messages, 8,849 symbols reconstructed
simultaneously — replays in 50 seconds with no invariant failure. The panel it
produces is 86.1 million book events across seven sessions and eight symbols,
and two results rest on it.

**Order flow imbalance predicts short-horizon mid-price changes, and cannot pay
for the spread.** The effect is statistically solid and economically
negligible: across all 144 specifications the predicted move stays below half
the quoted spread, reaching at best 19% of it. The gap between statistical and
economic significance is the finding.

**The queue at the best quote is not a homogeneous process.** Cancellation
intensity rises with queue size in every symbol while limit arrival stays close
to flat, and a state-dependent fit beats a constant-rate Poisson null on
held-out likelihood in 15 of 16 symbol-sides.

The research question, the evaluation protocol, the universe, and the things
this project deliberately refuses to do were all fixed in
[`DESIGN.md`](DESIGN.md) before any coefficient was computed.

## Does order flow imbalance predict returns?
...
Feature construction was settled and unit-tested before the first regression
ran.
...
- **Linear and univariate.** A single regressor with no interactions. The
  queue-reactive fit below is where a non-linear, state-dependent treatment
  starts; the Hawkes extension in `DESIGN.md` is where it would continue.
...
## Is the touch queue state-dependent?

A different question from the one above. Rather than predicting price, it asks
whether the queue at the best quote is a *homogeneous* process at all.
...
Both results rest on a book that was checked rather than assumed. A
reconstruction that is subtly wrong produces features that are subtly wrong and
a result that is confidently false, so the checks below come in two kinds:
internal ones, which ask whether the book agrees with itself, and one external
one, which asks whether it agrees with a second venue.

### A full session, every invariant armed
...
### The exported artifact is re-checked, not trusted

`py/validate_export.py` re-checks the exported files rather than trusting them:
1,120 checks across the 56 files, all passing. Timestamps monotone, coverage
running from the opening bell to the closing one, no crossed book, price and
size agreeing about whether a side is empty, and, on *both* sides
independently, that adds move the touch by their own size, that cancels and
trades remove their own size, and that replaces decompose into their two legs.
...
Re-exporting a session reproduces all eight files byte-for-byte, and the row
counts match [`PANEL.md`](PANEL.md). The reproducibility claim there is tested,
not asserted.

### Two venues agree

Every check above is internal, and all of them would pass on a book that is
...
events; Python computes features. Feature definitions change many times during
a study, and each change should cost a re-read of these files rather than a
re-parse of a multi-gigabyte session. The same artifact then serves both the
regression and the point-process fit, which want very different views of the
same events.

Exporting AAPL, MSFT, and SPY from the full Nasdaq session takes 24.6 s and
produces 4.37 M rows.
...
regression belongs on a construction that is already settled.
...
scripts/build_dataset.sh  fetches, exports and validates the whole panel
data/                     gitignored; fetched, never committed
```

```
py/features.py              OFI, queue imbalance, aggressor sign, spreads
py/build_buckets.py         event panel to k-event bucket aggregates
py/study_ofi.py             impact and prediction, with the cost comparison
py/queue_states.py          touch-queue transitions from the event log
py/build_queue_states.py    the same, cached for the whole panel
py/queue_reactive.py        the fitted intensity model and its Poisson null
py/study_queue_reactive.py  held-out likelihood and the generative test
py/validate_export.py       re-checks an exported session
py/cross_venue_check.py     Nasdaq against Nasdaq BX
py/figures.py               every figure, light and dark
```
...
Seven distinct days matters for the study. Out-of-sample means a different
day, not a later slice of the same one; intraday autocorrelation and a shared
regime make same-day holdout optimistic by construction.
